    ...
    User.findOne({email: req.body.email}, function (err, usr) {
      if (err) {
        ...
        const newUser = new User({
          email: req.body.email, password: req.body.password, mobile: req.body.phone, pin: req.body.pin
        });
  ...
  async.waterfall([function (done) {
    ...
    const smtpTransport = nodemailer.createTransport({
      service: 'SendGrid', auth: {
        user: 'apikey', pass: process.env.SENDGRID_SECRET,
      }
    });
...
router.get('/getFeedback/:email/:feedback', function (req, res) {
  const feedback = new Feedback({
    email: req.params.email, feedbackText: req.params.feedback
  });
